...
forward = partial(fmap.shifted_log_forward, shift=shift)
backward = partial(fmap.shifted_log_backward, shift=shift)

def non_lin(x):
	return tf.nn.relu(x+1.0)-1.0

time_str = 'shifted_log_{}_limit_non_lin{}'.format(shift, Mpch)
