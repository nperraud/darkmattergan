...
def plot_array_pil(x, pil_object=True, lims=None, log_input=False, log_clip=1e-7):
...
def plot_with_shade(ax, x, y, label, color, **linestyle):
    ...
    ax.fill_between(x, y_mean - error, y_mean + error, alpha=transparency, color=color)
...
def plot_array_plt(x, ax=None, cmap='planck', color='black', simple_k = 10):
    ...
    plt.tick_params(axis='both',  # changes apply to the x-axis
                    ...
                    left='off', right='off', labelbottom='off', labelleft='off')  # labels along the bottom edge are off
    try:
    ...
    im = plt.pcolormesh(np.reshape(log_x, [size, size]), cmap=cmap, vmin=lims[0], vmax=lims[1], edgecolors='face')
...
def planck_cmap(ncolors=256):
    ...
    """
    segmentdata = {
        "red": [(0.0, 0.00, 0.00), (0.1, 0.00, 0.00), (0.2, 0.00, 0.00), (0.3, 0.00, 0.00), (0.4, 0.00, 0.00),
                (0.5, 1.00, 1.00), (0.6, 1.00, 1.00), (0.7, 1.00, 1.00), (0.8, 0.83, 0.83), (0.9, 0.67, 0.67),
                (1.0, 0.50, 0.50)],
        "green": [(0.0, 0.00, 0.00), (0.1, 0.00, 0.00), (0.2, 0.00, 0.00), (0.3, 0.30, 0.30), (0.4, 0.70, 0.70),
                  (0.5, 1.00, 1.00), (0.6, 0.70, 0.70), (0.7, 0.30, 0.30), (0.8, 0.00, 0.00), (0.9, 0.00, 0.00),
                  (1.0, 0.00, 0.00)],
        "blue": [(0.0, 0.50, 0.50), (0.1, 0.67, 0.67), (0.2, 0.83, 0.83), (0.3, 1.00, 1.00), (0.4, 1.00, 1.00),
                 (0.5, 1.00, 1.00), (0.6, 0.00, 0.00), (0.7, 0.00, 0.00), (0.8, 0.00, 0.00), (0.9, 0.00, 0.00),
                 (1.0, 0.00, 0.00)]}
...
def plot_images_psd(images, title, filename = None, sigma_smooth = None):
    ...
    fig = plt.figure(figsize=(n_cols * m, n_rows * m), dpi=my_dpi)
    ...
    for i in range(n):
        ...
        plot_with_shade(ax1, k, ps_real, color='b', label="Real $\mathcal{F}(X))^2$", **linestyle)
    ...
    if filename is not None:
        ...
        plt.savefig(filename, bbox_inches='tight', dpi=my_dpi )#, bbox_extra_artists=(txt_top)) #, txt_left))  # Save Image
    plt.show() 
