...
def plot_mass_hist_diffs(data_list, labels, colors, lim, params):
    ...
    for i in range(nc-1):
        ...
        plt.xscale("log")
        for j in range(len(data_list)):
            hist, bins, _ = metrics.mass_hist(dat=np.clip(data_list[j][(i+1)::nc]-data_list[j][i::nc], a_min=0, a_max=None), lim=None)
            plt.plot(bins, hist, '-', label=labels[j], c=colors[j])
        plt.legend()
    limneg = (0, lim[1])
    for i in range(nc-1):
        ...
        plt.xscale("log")
        for j in range(len(data_list)):
            hist, bins, _ = metrics.mass_hist(dat=np.clip(data_list[j][i::nc]-data_list[j][(i+1)::nc], a_min=0, a_max=None), lim=None)
            plt.plot(bins, hist, '-', label=labels[j], c=colors[j])
        plt.legend()
